...
async function main() {
    try {
        ...
        const clientPrivateKey = "0xYourClientPrivateKey";
        ...
        const channelId = clientSigner.address;
        // Stato iniziale off-chain: deve rispecchiare il deposito on-chain (es. 1.0 ETH)
        const deposit = ethers.parseEther("1.0").toString();
        const initialState = {
            nonce: 0,
        ...
        if (onChainDeposit?.toString() !== deposit) {
            throw new Error("On-chain deposit does not match expected off-chain channel balance.");
        }
        ...
        async function sendMicropayment(api, channelId, amountEther) {
            const currentState = api.currentState;
            if (!currentState)
                throw new Error("Current state not available");
            ...
            console.log(`Micropayment of ${amountEther} ETH sent. Updated state:`, newState);
            return statePackage;
        }
        ...
        console.log("Client: waiting for relay's signature to finalize the channel...");
