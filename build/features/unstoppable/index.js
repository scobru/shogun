...
export default class UnstoppableChat {
    ...
    async validatePubKeyFromUsername(username, pubKey) {
        const gun = this.gun;
        let verified = false;
    ...
    async join(username, password, publicName) {
        const gun = this.gun;
    ...
    async loadMessagesOfContact(pubKey, publicName) {
        const gun = this.gun;
        if (!gun.user().is)
            return;
            ...
            path.on((msgs) => {
                ...
                Object.keys(msgs).forEach((time) => {
                    ...
                    path.get(time).on(async (msgDataString) => {
                        if (thisChat.activeContact !== pubKey ||
                            !msgDataString ||
                            msgDataString === 'null' ||
                            loadedMsgs[time])
                            return;
        ...
        loadMsgsOf(gun.user(pubKey).get('pchat').get(gun.user()._.sea.pub), publicName, emitter);
        return {
            on: (cb) => {
    ...
    async createChannel(channelName, isPrivate) {
        const gun = this.gun;
        if (!gun.user().is)
            return;
                    ...
                    gun
                        ...
                        if (channel.disabled && loadedChannels[channelKey]) {
                            ...
                            gun
                                ...
                                .get('peers')
                                .once(async (peers) => {
                                if (!peers || loadedChannels[channelKey])
                                    return;
    ...
    loadPublicChannels() {
        ...
        return {
            on: (cb) => {
                ...
                gun.get('#public/channels/0').on((hashMap) => {
    ...
    async joinPublicChannel(publicChannel) {
        const gun = this.gun;
        if (!gun.user().is)
            return;
        const publicName = this.publicName;
        const pubKey = gun.user().is.pub;
        if (!publicName || !pubKey || !publicChannel)
            return;
    ...
    async inviteToChannel(channel, username, peerPubKey, publicName) {
        ...
        const eInvitePair = await Gun.SEA.encrypt(JSON.stringify(channel.pair), inviteSec);
        ...
        gun
            ...
            Object.keys(peerInvites).forEach((peerPub) => {
                ...
                gun
                    ...
                    .get(peerPub)
                    .on(async (channels) => {
                    if (!channels || channels === 'disabled')
                        return;
                    Object.keys(channels).forEach(async (channelKey) => {
                        const channel = typeof channels[channelKey] === 'string' &&
    ...
    async sendMessageToChannel(channel, msg, peerInfo) {
        ...
        channelChatToSend.get(time).put(JSON.stringify({
            msg: msgToSend,
    ...
    async loadMessagesOfChannel(channel) {
        const gun = this.gun;
        if (!gun.user().is)
            return;
        ...
        async function loadMsgsOf(path, name, passedEmitter) {
            ...
            path.on((peerMsgs) => {
                ...
                Object.keys(peerMsgs).forEach((time) => {
                    if (loadedMsgs[time + name] || time === '_')
                        return;
                    path.get(time).on(async (msgDataString) => {
                        if (thisChat.activeChannel !== channel.key ||
    ...
    async createAnnouncement(announcementName, isPrivate, rssLink) {
        const gun = this.gun;
        const publicName = this.publicName;
        ...
        if (isPrivate) {
            ...
            const hash = await Gun.SEA.work(JSON.stringify(announcement), null, undefined, { name: 'SHA-256' });
            if (!hash)
                return;
            gun
    ...
    leaveAnnouncement(announcement) {
        const gun = this.gun;
        const leaveMsg = `${this.publicName} has left the chat.`;
                            ...
                            gun
                                ...
                                gun
                                    ...
                                    Object.keys(admins).forEach((pubKey) => {
                                        if (pubKey === '_' || loadedAdmins[pubKey])
                                            return;
                                        gun
                                            .user()
                                            .get('announcement')
    ...
    loadPublicAnnouncements() {
        ...
        return {
            on: (cb) => {
                ...
                gun.get('#public/announcements/3').on((hashMap) => {
                    Object.keys(hashMap).forEach((hash) => {
                        if (hash === '_') {
                            return;
                        }
    ...
    async joinPublicAnnouncement(publicAnnouncement) {
        const gun = this.gun;
        const publicName = this.publicName;
        const pubKey = gun.user().is.pub;
        if (!publicName || !pubKey || !publicAnnouncement)
            return;
    ...
    async inviteToAnnouncement(announcement, username, peerPubKey, publicName) {
        ...
        const inviteSec = await Gun.SEA.secret(otherPeerEpub, gun.user()._.sea);
        const eInvitePair = await Gun.SEA.encrypt(JSON.stringify(announcement.pair), inviteSec);
        ...
        gun
            ...
            Object.keys(peerInvites).forEach((peerPub) => {
                ...
                gun
                    ...
                    .get(peerPub)
                    .on(async (announcements) => {
                    if (!announcements || announcements === 'disabled')
                        return;
                    Object.keys(announcements).forEach(async (announcementKey) => {
                        const announcement = typeof announcements[announcementKey] === 'string' &&
                            announcements[announcementKey] !== 'disabled'
                            ? JSON.parse(announcements[announcementKey])
                            : announcements[announcementKey];
                        if (announcementKey === '_' ||
                            !announcement ||
                            (announcement &&
                                announcement.key &&
    ...
    async acceptAnnouncementInvite(invite) {
        ...
        Object.keys(invite.admins).forEach((pubKey) => {
            ...
            if (loadedAdmins[pubKey] || !admin || admin === 'disabled')
                return;
    ...
    async sendMessageToAnnouncement(announcement, msg, peerInfo) {
        ...
        const announcementChatToSend = gun
            ...
            .get(announcement.key)
            .get('chat');
        announcementChatToSend.get(time).put(JSON.stringify({
            msg: msgToSend,
            userPub: gun.user().is.pub,
            userName: this.publicName,
            time,
            ...
            path.on((peerMsgs) => {
                ...
                Object.keys(peerMsgs).forEach((time) => {
                    if (loadedMsgs[time + name] || time === '_')
                        return;
                    path.get(time).on(async (msgDataString) => {
                        if (thisChat.activeAnnouncement !== announcement.key ||
                            loadedMsgs[time + name])
                            return;
                        loadedMsgs[time + name] = true;
                        let msgToLoad;
                        ...
                        if (msgData.peerInfo) {
                            ...
                            else if (msgData.peerInfo.action === 'newAdmin' &&
