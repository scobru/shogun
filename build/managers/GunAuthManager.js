 ...
 */
export class GunAuthManager extends BaseManager {
    constructor(gun, APP_KEY_PAIR) {
        super(gun, APP_KEY_PAIR);
        this.isAuthenticating = false;
        this.pub = "";
        this.storagePrefix = "auth";
        this.pub = "";
    ...
    async getPubFromAlias(alias) {
        return new Promise((resolve) => {
            this.gun.get(`~@${alias}`).once((data) => {
                if (!data)
                    return resolve(null);
                if ("_" in data) {
                    delete data._;
     ...
     */
    async createAccount(alias, passphrase) {
        const MAX_RETRIES = 5;
        let attempts = 0;
        ...
        while (attempts < MAX_RETRIES) {
            try {
                ...
                const userExists = await Promise.race([
                    this.exists(alias),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout verifica utente")), 10000)),
                ]);
                if (userExists) {
                ...
                const result = await Promise.race([
                    new Promise((resolve, reject) => {
                        this.user.create(alias, passphrase, (ack) => {
                            if (ack.err) {
     ...
     */
    async importGunKeyPair(keyPairJson) {
        let keyPair;
        try {
